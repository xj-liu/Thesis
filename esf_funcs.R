...
# @cvmethod: either "CV" or "SpCV" indicating the type of cross-validation
# This function returns the LASSO coefficients corresponding to the eigenvector features
esf_coeffs <- function(eigens.df, target, coor, folds = 20, cvmethod){
...
# @lasso.cv: either "CV" or "SpCV" indicating the type of cross-validation for LASSO
...
hold_eval <- function(param, train.id, test.id, target.var, data.sf, 
                      lasso.cv = "SpCV", lasso.fold = 20, prox = TRUE, 
